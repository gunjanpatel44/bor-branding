        ...
        <div key={section.id} className="border-b border-gray-800">
          ...
          <div
            className={`grid transition-all duration-500 ease-in-out text-gray-400 ${
              openPanel === section.id ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'
            }`}
          >
            <div className="overflow-hidden leading-relaxed">
              {Array.isArray(section.desc) ? (
                <ul className="list-disc pb-5 list-inside space-y-1">
                  {section.desc.map((item, idx) => (
                    <li key={idx}>{item}</li>
                  ))}
                </ul>
              ) : (
                <p className="pb-5">{section.desc}</p>
              )}
            </div>
          </div>
        </div>
